# m_detector/launch/detector_ouster_bag.launch.py
...
def generate_launch_description():
    ...
    time_file_arg = DeclareLaunchArgument(
        "time_file", default_value="", description="Time log output file"
    )
    out_path_arg = DeclareLaunchArgument(
        "out_path", default_value="", description="Detector output file"
    )
    out_origin_path_arg = DeclareLaunchArgument(
        "out_origin_path", default_value="", description="Origin output file"
    )
    pose_log_arg = DeclareLaunchArgument(
        "pose_log", default_value="false", description="Enable pose logging"
    )
    pose_log_file_arg = DeclareLaunchArgument(
        "pose_log_file", default_value="", description="Pose log output file"
    )
    cluster_out_file_arg = DeclareLaunchArgument(
        "cluster_out_file", default_value="", description="Cluster output file"
    )
    time_breakdown_file_arg = DeclareLaunchArgument(
        "time_breakdown_file", default_value="", description="Time breakdown output file"
    )
    ...
    ouster_config_yaml = PathJoinSubstitution(
        [pkg_share, "config", "ouster", "ouster_station.yaml"]
    )
